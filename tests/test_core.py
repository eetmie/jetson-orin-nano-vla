...
class ParityTests(unittest.TestCase):
    ...
    def test_different_model_families_are_not_comparable(self):
        zeros = np.zeros((2, 2, 2), dtype=np.float32).tolist()
        reference = signed_result("reference", zeros)
        candidate = signed_result("candidate", zeros)
        candidate["model"]["family"] = "xvla"
        self.assertEqual(compare(reference, candidate)["verdict"], "NOT COMPARABLE")
